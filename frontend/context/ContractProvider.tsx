...
export function useRewardBuyCollectibleBatch() {
  const chainId = useChainId();
  const contractAddress = REWARD_CONTRACT_ADDRESSES[chainId];
  const { writeContractAsync, isPending, error: writeError, data: txHash, reset } = useWriteContract();
  const { isLoading: isConfirming, isSuccess } = useWaitForTransactionReceipt({ hash: txHash });

  const buyBatch = useCallback(async (tokenIds: bigint[], useUsdc = false) => {
    if (!contractAddress) throw new Error('Reward contract not deployed');
    if (!tokenIds?.length) throw new Error('Empty token list');
    return await writeContractAsync({
      address: contractAddress,
      abi: RewardABI,
      functionName: 'buyCollectibleBatch',
      args: [tokenIds, useUsdc],
    });
  }, [writeContractAsync, contractAddress]);

  return { buyBatch, isPending: isPending || isConfirming, isSuccess, isConfirming, error: writeError, txHash, reset };
}
