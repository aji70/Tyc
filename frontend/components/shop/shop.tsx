...
import {
  ...
  useDojoRewardBuyCollectibleBatch,
...
// Perks 11–14 (Rent Cashback, Interest, Lucky 7, Free Parking) are valid on-chain; show as "Coming soon" until stocked. Contract accepts any perk 1–255.
const COMING_SOON_PERK_IDS = [11, 12, 13, 14];

// Default bundles shown in UI (used when API returns empty or before migration). Include token_ids when known to enable on-chain bundle buy.
const DEFAULT_BUNDLES: Array<{ id?: number; name: string; description: string | null; price_tyc: string; price_usdc: string; price_ngn?: number | null; token_ids?: number[] }> = [
...
export default function GameShop() {
  ...
  const [bundles, setBundles] = useState<Array<{ id: number; name: string; description: string | null; price_tyc: string; price_usdc: string; price_ngn?: number | null; token_ids?: number[] }>>([]);
  ...
  // Starknet/Dojo: allowance and balance stubbed (Dojo buy handles token transfer)
  const tycAllowance = undefined as bigint | undefined;
  const usdcAllowance = undefined as bigint | undefined;
  ...
  const approve = async (_contractAddress: unknown, _spender: unknown, _amount: bigint) => undefined;
  ...
  const tycLoading = false;
  const usdcLoading = false;
  const refetchTyc = () => {};
  const refetchUsdc = () => {};
  const tycBalance = '0.00';
  ...
  const shopItems = useMemo(() => {
    if (!shopInfoResults || shopInfoResults.length === 0) return [];

    return shopInfoResults
      .map((result: { status: string; result?: [number, bigint, bigint, bigint, bigint] }, index: number) => {
  ...
  // ── User Vouchers (Starknet/Dojo: stubbed until useDojoUserVouchers) ──
  const userVoucherIds: bigint[] = [];
  const voucherInfoResults: { status: string; result?: [number, bigint, bigint, bigint, bigint] }[] | undefined = undefined;
  ...
  const handleBuyBundle = async (b: { token_ids?: number[]; price_usdc: string; name: string }) => {
    if (!isConnected || !address) {
      toast.error('Please connect your wallet');
      return;
    }
    if (!b.token_ids?.length) {
      toast.error('This bundle is not yet available for on-chain purchase');
      return;
    }

    const useUsdc = true;
    const priceNum = Number(b.price_usdc);
    const balanceNum = Number(usdcBalance);
    if (balanceNum < priceNum) {
    ...
    const tokenIds = b.token_ids.map((id) => BigInt(id));
    const price = BigInt(Math.round(priceNum * 1e6));

    const allowance = usdcAllowance;
    if (!usdcTokenAddress || !contractAddress) {
      toast.error('Token or contract not supported on this network');
      return;
    }

    try {
      if (allowance === undefined || allowance === null || (typeof allowance === 'bigint' && allowance < price)) {
        toast.info('Approval required for bundle');
        await approve(usdcTokenAddress, contractAddress, price);
        toast.success('Approval successful, completing bundle purchase...');
      }
      await buyBatch(tokenIds, useUsdc);
    } catch (err: unknown) {
  ...
  }, [batchSuccess, refetchTyc, refetchUsdc, resetBatch]);
  ...
  useEffect(() => {
    ...
    if (batchError) toast.error(batchError.message || 'Bundle purchase failed');
    if (redeemError) toast.error(redeemError.message || 'Redemption failed');
  }, [buyError, batchError, redeemError]);
  ...
  useEffect(() => {
    apiClient.get<{ success?: boolean; ngn_available?: boolean; bundles?: Array<{ id: number; name: string; description: string | null; price_tyc: string; price_usdc: string; price_ngn?: number | null; token_ids?: number[] }> }>('shop/bundles').then((r) => {
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      {b.token_ids?.length ? (
                        <button
                          ...
                          disabled={batchPending || batchConfirming || Number(usdcBalance) < Number(b.price_usdc)}
                          className="w-full py-3 rounded-xl font-semibold bg-gradient-to-r from-[#00F0FF] to-[#0DD6E0] text-black hover:shadow-[0_0_20px_rgba(0,240,255,0.3)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                          {batchPending || batchConfirming ? (
                            <><Loader2 className="w-5 h-5 animate-spin" /> Purchasing bundle...</>
                          ) : (
                            <><CreditCard className="w-5 h-5" /> Buy bundle with USDC</>
                          )}
                        </button>
                      ) : (
                        <button
